#!/usr/bin/env python3
"""
Thoth GUI Application

Main PyQt6 application for the Thoth historical handwriting recognition tool.
...
class MainWindow(QMainWindow):
    """Main application window for Thoth."""
    ...
    def init_ui(self) -> None:
        ...
        self.setWindowTitle("Thoth - Historical Handwriting Recognition")
        ...
        # Use a vertical layout so we can add a branded header above the main splitter
        ...
        logo_widget = self._create_header_logo_widget(height=48)
    ...
    def on_about(self) -> None:
        """Show about dialog."""
        about_title = "About Thoth"
        logo_path = self._resolve_brand_icon_path()
        logo_img_tag = f"<img src='file:///{logo_path.as_posix()}' height='48'/>" if logo_path else ""
        QMessageBox.about(
            self,
            about_title,
            (
                f"{logo_img_tag}<h3>Thoth</h3>"
                "<p>An AI-powered application for transcribing historical handwriting<br>"
                "using state-of-the-art machine learning models.</p>"
                "<p><b>Version</b>: 0.1.0<br>"
                "Built with PyQt6 and Hugging Face Transformers</p>"
                "<p><b>Features</b>:")
            + "<ul>"
            + "<li>TrOCR-based handwriting recognition</li>"
            + "<li>LoRA fine-tuning for efficiency</li>"
    ...
    def _apply_branding(self) -> None:
        """Apply app/window icon and any global brand tweaks."""
    ...
    def _resolve_brand_icon_path(self) -> Optional[Path]:
        """Find a suitable brand icon file on disk.

        Tries multiple candidate locations and filenames.
        """
        candidates = []
        here = Path(__file__).resolve()
        # dev/assets/new-logos
        dev_assets = here.parents[2] / "assets" / "new-logos"
        brand_kit = here.parents[3] / "brand kit" / "new-logos"
        for base in (dev_assets, brand_kit):
            # Prefer the cropped primary lockup first (SVG), then other options
            candidates.extend([
                base / "PRIMARY-LOCKUP-cropped.svg",
                base / "PRIMARY-LOCKUP.svg",
                base / "type-logo.svg",
                base / "type-logo.png",
                base / "logo-no-background.png",
                base / "logo.png",
                base / "PRIMARY-LOCKUP-NO-BACKGROUND.png",
            ])
    ...
    def _create_header_logo_widget(self, height: int = 48) -> Optional[QWidget]:
        """Create a widget to render the brand logo (SVG or raster)."""
        icon_path = self._resolve_brand_icon_path()
        if not icon_path:
            return None
        suffix = icon_path.suffix.lower()
        if suffix == ".svg":
            svg = QSvgWidget(str(icon_path))
        ...
        if pix.isNull():
            return None
        label.setPixmap(pix.scaledToHeight(height, Qt.TransformationMode.SmoothTransformation))
        return label
